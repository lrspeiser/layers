  ...
  return <section className="record-comparison-workbench">
    ...
    <div className="record-comparison-viewer">
      {pixelsAvailable ? <>
        ...
        {view === "swipe" && <span className="swipe-mask-legend"><i className="rubin-mask-swatch" /> Rubin masked <i className="reference-mask-swatch" /> {reference?.survey} masked</span>}
      </> : <div className="qa-pixel-fallback"><span className="eyebrow">PUBLIC RUBIN PREVIEW UNAVAILABLE</span><h3>The display image failed to load on this host.</h3><p>The comparison metadata, masks, coverage, provenance, and checksums remain public; the full calibrated FITS planes are retained in analysis storage.</p></div>}
    </div>
