...
the Legacy r band to the same PSF. It measures 6.4-arcsec spatial cells inside
...
def audit_target(slug: str, coverage: dict, comparisons: Path, legacy_root: Path) -> dict:
    ...
    if reconciliation.get("layerIds", [None, None])[1] != "legacy-survey-dr10" or reconciliation.get("band") != "z":
        return {
            "schemaVersion": 1,
            "objectId": slug,
            "status": "blocked",
            "reason": "The current resolved transfer test supports Legacy r-z to Rubin z only.",
        }
    ...
    legacy_r_path = legacy_root / slug / "legacy_r.fits"
    ...
    audit = {
        ...
        "method": "6.4-arcsec resolved cells within 1.5 times the catalogued semi-major axis; Legacy r matched to the z-pair PSF and sky model",
        ...
        "supportingProducts": {
            ...
            "legacyRKernelSigmaPixels": kernel_sigma,
            "legacyRKernelRadiusPixels": kernel_radius,
        },
...
def main() -> None:
    ...
    summary = []
    for slug in slugs:
        result = audit_target(slug, coverage, args.comparisons, args.legacy_root)
